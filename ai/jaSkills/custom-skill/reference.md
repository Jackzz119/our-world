...
当前 skill 列表：
!for f in ai/jaSkills/*/SKILL.md; do echo $(basename $(dirname $f)); done
```
...
| 类型 | 存放位置 | 示例 |
|------|----------|------|
| 通用 skill | `ai/jaSkills/` (项目内) 或 `~/.claude/skills/` (全局) | vc, custom-skill |
| 项目特定 skill | `ai/jaSkills/` (项目内) | feature (含项目文档路径) |
