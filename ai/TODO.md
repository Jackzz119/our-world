...
- [ ] **世界设置弹窗没有入口**（PA-037；纪念日为 null 时日历回退假日期；**用户 2026-09-19 决定暂不做，先登记**）：`world-settings.tsx` 有真实的 DB 保存链（`updateWorld`），但 `screen='world-settings'` 在当前 UI 里无任何按钮可达（原 sidebar 入口随 Discord 壳层退役）。需产品决定入口位置（设置弹窗内的「我们的小世界」段 / 导航工具菜单），随 E1-U M4 落地
