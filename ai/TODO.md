...
- [ ] **[BUG] UI 控件状态与真实行为不符**：迷你音乐播放/进度仍需随 M3 修正；设置密码/邮箱/应用锁假反馈已于 2026-09-19 处理，M4 保留真实能力；超出 UI 的功能接通另拆任务
- [ ] **[BUG] 窄屏浮窗越界和重叠**：390px 下音乐条越界并被底导航遮挡，天气与纪念卡重叠；随 M3 停靠规则处理
- [x] **[BUG] 设置里「保存新密码」是假成功**（2026-09-19 修，PA-035：改密接 `updateUser` 并先验当前密码，邮箱只读，应用锁删除，退出可重试）（审核第三步 PA-035 扩展：同组邮箱行是 localStorage 假值、应用锁无人执行、退出忽略 `signOut` 错误会卡死）：`settings.tsx` 的 `savePw` 只把本地状态置为「已更新」，从未调用 `supabase.auth.updateUser`，密码实际未改（2026-09-19 审核第二步确认）。修法：接 `updateUser({ password })`，失败显示错误、成功后清空输入；同段「绑定邮箱」「应用锁」同属占位，一并接通或收起
- [ ] **世界设置弹窗没有入口**（PA-037；纪念日为 null 时日历回退假日期；**用户 2026-09-19 决定暂不做，先登记**）：`world-settings.tsx` 有真实的 DB 保存链（`updateWorld`），但 `screen='world-settings'` 在当前 UI 里无任何按钮可达（原 sidebar 入口随 Discord 壳层退役）。需产品决定入口位置（设置弹窗内的「我们的小世界」段 / 导航工具菜单）；暂不随 E1-U M4 落地
- [ ] **审核第三步发现（2026-09-19，诊断未修，详见 `ai/project-audit/FINDINGS.md` PA-034～053）**：
    ...
    - [x] **[BUG] 房间装载期四处**（2026-09-19 修，PA-041；DPR>1 屏与断图实测待做）：HiDPI 下房间缩到左上角露底、装载期间 mood/天气变更被丢、任一贴图 404 整间房静默空白、装载中回大厅报 TypeError（PA-041）
    - [x] **[BUG] 聊天呈现**（2026-09-19 修，PA-038；双端实测待做）：对方头顶气泡在我 4.5s 内回复时挂住不消失；rail 红点无视已读、关窗回亮（PA-038）
    - [x] **[BUG] 聊天一致性**（2026-09-19 修，PA-039；双端断网实测待做）：断线期间对方的删除/撤 reaction 重连后补不回；编辑/删除/reaction/已读失败不回滚无提示；重试已落库消息永远卡 failed（PA-039）
    - [x] **[BUG] 上传管线**（2026-09-19 修，PA-040；世界 icon 同型问题 A-16 未改）：选图放行 gif/heic 到桶才拒、无 25MB 预检、部分成功后失败留 Storage 孤儿（PA-040；含「回忆链路边界测试」第一条文案 bug）
    - [x] 昵称写回 `display_name`（PA-036）· 账号设置假反馈（PA-035）——2026-09-19 修；世界设置入口（PA-037）用户决定暂不做
...
- [ ] **M1 静态标准板定稿**（进行中，2026-09-20）：[实景标准板](design_system/uiux/cinnaglass/standard-board.html) 与 [B1/B2 并排比稿](design_system/uiux/cinnaglass/material-comparison.html) 已提供；导航/房间/天气复用真实组件，聊天/音乐/设置为本地样板。待用户确认正文材质、A/B 厚度、文字/控件与纹理取样，再进入产品迁移
- [ ] **M2 公共基础与弹层**：A/B 作用域、共享控件、统一层级/关闭/焦点/隐藏机制；导航视觉保持，日记依赖隔离
- [ ] **M3 A 场景悬浮 UI 迁移**：天气、聊天、音乐两态、纪念卡、头顶状态；真实播放状态、隐藏恢复、窄屏避让一起处理
- [ ] **M4 B 任务界面迁移**：设置先作样板，再统一日历/时钟/完整聊天/选择器/照片与心愿通用壳；世界设置入口 PA-037 按 2026-09-19 决定暂缓；收起旧社交导航，保留真实数据功能
...
- [ ] **Supabase 审计遗留 migration**（要点见 PROJECT.md 数据库章节 + `supabase.md` 三/四章）：⚠️ worlds 外键 CASCADE→SET NULL（member 删号不该灭世界）+ 安全包（search_path/revoke/GraphQL/泄露密码保护）+ 性能包（RLS initplan×13 + FK 索引×4）；顺手加 `channel_reads.last_read_message_id`（已读游标改按消息 ID，替代时间比较；2026-09-19 用户提出）
- [x] 昵称编辑写回 `profiles.display_name`（2026-09-19 修，PA-036：`updateMyDisplayName`，「她」只读；线上 RLS 实测待做）
...
- [~] 回忆链路边界测试（2026-07-08 实测，2026-08-21 复核仍成立；2026-09-19 审核第三步再核，两条文案 bug 根因见 PA-034 / PA-040）：非图片 mime 被过滤 ✓、超 25MB 上传失败且草稿保留 ✓、断网失败且草稿保留 ✓、签名过期由 40 分钟续签覆盖 ✓。**剩两处文案 bug 待修**（2026-09-18 核对源码仍未修）：
    - ~~超限上传把 Storage 英文原文直接抛给用户~~ 2026-09-19 已修（PA-040：选图时按格式/25MB 预检并中文提示；服务端其他报错的中文映射归 PA-043）
