...
const args = Object.fromEntries(
    process.argv.slice(2).reduce((p, v, i, a) => (i % 2 === 0 && p.push([v.replace(/^--/, ''), a[i + 1]]), p), [])
);
if (!args.root || !args.output)
    throw new Error('用法：--root <项目根> --output <输出目录> [--include src,scripts,vite.config.ts]');
...
for (const file of files) {
    ...
    const record = (name, node, type) => {
        ...
        decls.push({
            ...
            lines: end - start + 1,
            exported,
            commented: hasLeadingComment(node),
            bodyHash: hash
        });
    };
    /** 遍历顶层与嵌套声明。 */
    const visit = (node) => {
        if (
            ...
            if (spec) {
                ...
                edges.push({
                    from: file,
                    spec,
                    typeOnly: !!(ts.isImportDeclaration(node) && node.importClause?.isTypeOnly),
                    dynamic: !ts.isImportDeclaration(node),
                    ...r
                });
            }
        }
        if (
            ...
            (ts.canHaveModifiers(node) && ts.getCombinedModifierFlags(node) & ts.ModifierFlags.Export)
        )
            exportsCount++;
        if (ts.isFunctionDeclaration(node) && node.name) record(node.name.text, node, 'function');
            ...
            record(node.name.text, node, 'method');
                ...
                if (
                    ...
                    record(
                        d.name.text,
                        {
                            ...
                            getEnd: () => node.getEnd(),
                            getFullStart: () => node.getFullStart(),
    ...
    while ((tok = scanner.scan()) !== ts.SyntaxKind.EndOfFileToken)
        if (tok === ts.SyntaxKind.SingleLineCommentTrivia || tok === ts.SyntaxKind.MultiLineCommentTrivia) comments++;
    modules.push({
        file,
        lines,
        ...
        headerComment: commentRanges > 0
    });
}
const internal = edges.filter((e) => e.internal);
const fanOut = Object.fromEntries(
    files.map((f) => [f, new Set(internal.filter((e) => e.from === f).map((e) => e.internal)).size])
);
const fanIn = Object.fromEntries(
    files.map((f) => [f, new Set(internal.filter((e) => e.internal === f).map((e) => e.from)).size])
);
/** Tarjan 求强连通分量，输出规模 >1 的环。 */
const adj = new Map(
    files.map((f) => [f, [...new Set(internal.filter((e) => e.from === f && !e.typeOnly).map((e) => e.internal))]])
);
let index = 0;
const idx = new Map(),
    low = new Map(),
...
const orphans = files.filter(
    (f) => fanIn[f] === 0 && !/^(src\/main\.tsx|vite\.config\.ts|eslint\.config\.js|scripts\/)/.test(f)
);
...
const summary = {
    ...
    largest: [...modules]
        ...
        .slice(0, 12)
        .map((m) => [m.file, m.lines]),
    fanInTop: Object.entries(fanIn)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 10),
    fanOutTop: Object.entries(fanOut)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 10),
    cycles,
    orphans,
    unresolvedImports: edges.filter((e) => e.unresolved).map((e) => `${e.from} → ${e.unresolved}`),
    externals: [
        ...new Set(
            edges
                .filter((e) => e.external)
                .map((e) =>
                    e.external
                        .split('/')
                        .slice(0, e.external.startsWith('@') ? 2 : 1)
                        .join('/')
                )
        )
    ].sort(),
    declarations: decls.length,
    declarationsWithoutComment: decls.filter((d) => !d.commented).length,
    longFunctions: decls
        .filter((d) => d.lines > 80)
        .sort((a, b) => b.lines - a.lines)
        .map((d) => [`${d.file}:${d.start}`, d.name, d.lines]),
    duplicateBodyCandidates: dupBodies,
    method: 'TypeScript AST；导入按相对路径与 @/ 别名解析；同体候选按去注释规范化打印哈希，仍须人工语义复核；不能替代跨模块阅读'
};
writeFileSync(
    path.join(out, 'structure-modules.json'),
    JSON.stringify(
        modules.map((m) => ({ ...m, fanIn: fanIn[m.file], fanOut: fanOut[m.file] })),
        null,
        1
    )
);
    ...
    JSON.stringify(
        {
            ...
            unresolved: summary.unresolvedImports.length,
            decls: summary.declarations,
            uncommented: summary.declarationsWithoutComment,
            dupBodies: dupBodies.length,
            longFns: summary.longFunctions.length
        },
        null,
        1
    )
