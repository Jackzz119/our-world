...
if (!args.root || !args.revision)
    throw new Error('用法：--root <项目根> --revision <提交> [--output <json>] [--files a,b]');
...
const files = (
    args.files
        ? args.files.split(',')
        : execFileSync('git', ['diff', '--name-only', args.revision, '--'], { cwd: root, encoding: 'utf8' })
              .trim()
              .split('\n')
...
const results = files.map((file) => {
    let before;
    try {
        before = execFileSync('git', ['show', `${args.revision}:${file}`], { cwd: root, encoding: 'utf8' });
    } catch {
        return { file, status: 'new-file' };
    }
    let after;
    try {
    ...
    JSON.stringify(
        {
            ...
            commentOnly: results.filter((r) => r.status === 'comment-only').length,
            codeChanged: results.filter((r) => r.status === 'code-changed').map((r) => r.file)
        },
        null,
        1
    )
