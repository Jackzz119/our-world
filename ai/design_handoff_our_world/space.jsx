// space.jsx — 空间 switcher opened from the minimap. Visual floor-plan of the
...
const SPACE_MOODLBL = { golden: "黄昏光", twilight: "暮色光", night: "夜灯" };
...
function SpaceScreen({ open, onClose, rooms, meRoom, enterSpace, profile }) {
  const herName = profile.her || "她", meName = profile.me || "我";
    ...
    <>
      ...
      <div className={`modal mini glass ${open ? "show" : ""}`} aria-hidden={!open}>
        <div className="modal-hd">
          <span className="si" style={{ background: "linear-gradient(135deg,#BFE6FA,#7CC6EC)" }}><IMapPin size={18} /></span>
          <h2>空间</h2>
          <button className="modal-x" onClick={onClose} aria-label="关闭"><IClose size={17} /></button>
        </div>
        <div className="modal-body">
          <div className="sp-hint"><span className="ic"><IMapPin size={14} /></span>轻点一个房间，就一起过去呀</div>
          <div className="sp-grid">
            {rooms.map((r) => {
              ...
              return (
                <div key={r.id} className={`sp-card ${isCur ? "cur" : ""}`} style={{ background: SPACE_TINT[r.id] || "linear-gradient(150deg,#CDE3F2,#9FC2DC)" }}
                  onClick={() => enterSpace(r)}>
                  <span className="glow" />
                  {isCur && <span className="sp-here-tag"><IHeart size={10} fill="currentColor" sw={0} />你在这里</span>}
                  <div className="sp-top">
                    <span className="sp-ic"><RI size={22} /></span>
                    {ppl.length > 0 && (
                      <span className="sp-who">{ppl.map((p) => <span key={p.id} className="m" style={{ background: p.color }}>{p.ini}</span>)}</span>
                    )}
                  </div>
                  <div className="sp-b">
                    <div className="sp-nm">{r.name}</div>
                    <div className="sp-note">{r.note}</div>
                    <span className="sp-mood"><ISun size={11} />{SPACE_MOODLBL[r.mood] || "光线"}</span>
                  </div>
                </div>
              );
            })}
