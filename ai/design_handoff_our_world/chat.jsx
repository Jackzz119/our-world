// chat.jsx — multi-conversation chat. Docks bottom-left, expands to fullscreen.
...
/* ── contacts & seed threads ──
   "me" is 知夏; 小满 is the partner. Others are family / friends / a shared group. */
const CONTACTS = [
  { id: "xm",  name: "小满", ini: "满", color: "linear-gradient(135deg,#F8C8D6,#EF9DB4)", lover: true, online: true, status: "在线 · 在想你",
    replies: ["在呢，刚刚还在想你", "今天也好想见你呀", "晚上一起看日落好不好", "我也是，远远地抱抱你", "嗯，都听你的", "在回家的路上啦，等我"] },
  { id: "mom", name: "妈妈", ini: "妈", color: "linear-gradient(135deg,#FBE6A8,#F1C75A)", online: false, status: "30 分钟前在线",
    replies: ["吃饭了没？别老熬夜", "周末回来吃饭吗，给你们留门", "天凉了，记得加衣服", "钱够不够花，跟妈说"] },
  { id: "yue", name: "阿乐", ini: "乐", color: "linear-gradient(135deg,#BFE6FA,#6FBCE8)", online: true, status: "在线",
    replies: ["哈哈哈太对了", "周末打球不？", "下次带嫂子一起呀", "我也刚到家", "可以可以，安排"] },
  { id: "grp", name: "周末野餐 🧺", ini: "餐", color: "linear-gradient(135deg,#C9E8C2,#86C99A)", group: true, online: true, status: "5 人 · 3 人在线",
...
function Chat() {
  ...
  const [full, setFull] = useCS(false);
  const [view, setView] = useCS("list"); // docked sub-view: list | thread
  const [activeId, setActiveId] = useCS("xm");
  const [threads, setThreads] = useCS(SEED_THREADS);
  const [unread, setUnread] = useCS({ mom: 1, grp: 2 });
  const [text, setText] = useCS("");
  const [emoOpen, setEmoOpen] = useCS(false);
  const [emoTab, setEmoTab] = useCS(EMOJI_TABS[0]);
  const [typingId, setTypingId] = useCS(null);
  const [size, setSize] = useCS(() => { try { return JSON.parse(localStorage.getItem("ow-chat-size") || "null") || { w: 384, h: 564 }; } catch (_) { return { w: 384, h: 564 }; } });
  const [rz, setRz] = useCS(false);
  const scrollRef = useCR(null);
  const inputRef = useCR(null);
  const idRef = useCR(1000);
  ...
  const startResize = (axis) => (e) => {
    ...
    const up = () => {
      ...
      setRz(false);
      setSize((s) => { try { localStorage.setItem("ow-chat-size", JSON.stringify(s)); } catch (_) {} return s; });
    };
  ...
  const send = (e) => {
    ...
    setTimeout(() => {
      ...
      const reply = typeof pick === "string"
  ...
  const lastTime = (c) => { const arr = threads[c.id] || []; return arr.length ? arr[arr.length - 1].time : ""; };

  const senderInfo = (m) => {
    if (!active.group) return { n: active.ini, c: active.color };
    const mem = active.members[m.sender] || { n: "?", c: "#9aa" };
  ...
  const WinControls = () => (
    <>
      ...
      <button className="chat-ico" onClick={() => setOpen(false)} aria-label="收起"><IClose size={16} /></button>
    </>
  );
  ...
  return (
    <>
      ...
      <div className={`chat glass ${open ? "open" : ""} ${full ? "full" : "dock"} ${showThread ? "view-thread" : "view-list"} ${rz ? "resizing" : ""}`}
        style={full ? undefined : { width: size.w, height: size.h }} aria-hidden={!open}>
        {!full && <>
        ...
        <div className="chat-panes">
              ...
              {CONTACTS.map((c) => (
                <div key={c.id} className={`chat-li ${activeId === c.id && showThread ? "active" : ""}`} onClick={() => openContact(c.id)}>
                  <span className={`li-ava ${c.group ? "grp" : ""}`} style={{ background: c.color }}>
                    {c.ini}{c.online && !c.group && <span className="on" />}
                  </span>
                  <div className="li-body">
                    <div className="li-top">
                      <span className="li-nm">{c.name}{c.lover && <IHeart className="hk" size={12} fill="currentColor" sw={0} />}</span>
          ...
          <section className="chat-thread">
            <div className="th-hd">
              {!full && <button className="th-back" onClick={() => setView("list")} aria-label="返回"><IChevron size={20} style={{ transform: "rotate(180deg)" }} /></button>}
              <span className={`th-ava ${active.group ? "grp" : ""}`} style={{ background: active.color }}>{active.ini}</span>
              <div className="th-meta">
                <div className="th-nm">{active.name}{active.lover && <IHeart className="hk" size={12} fill="#F39DB4" sw={0} style={{ marginLeft: 3, verticalAlign: -1 }} />}</div>
                <div className="th-st">{active.online && <i />}{active.status}</div>
              </div>
              ...
              <div className="chat-col">
                ...
                {(threads[activeId] || []).map((m) => {
                  ...
                  return (
                    <div className={`msg ${m.from}`} key={m.id}>
                      {m.from === "them" && <span className="ava-s" style={{ background: si.c.startsWith("#") ? si.c : si.c }}>{si.n}</span>}
                      <div className="bub-wrap">
                        {m.from === "them" && active.group && <div className="bub-sender">{si.full}</div>}
                ...
                {typingId === activeId && (
                  <div className="msg them">
                    <span className="ava-s" style={{ background: active.color }}>{active.ini}</span>
            ...
            <form className="chat-input" onSubmit={send}>
              <div className={`emo-pop glass ${emoOpen ? "show" : ""}`}>
                <div className="emo-tabs">
                  {EMOJI_TABS.map((t) => (
                    <button type="button" key={t} className={emoTab === t ? "on" : ""} onClick={() => setEmoTab(t)}>{t}</button>
                  ))}
                </div>
                <div className="emo-grid">
                  {EMOJI[emoTab].map((e, i) => (
                    <button type="button" key={e + i} onClick={() => addEmoji(e)} aria-label={e}>{e}</button>
                  ))}
                </div>
              </div>
              <div className="chat-inwrap">
                ...
                <input ref={inputRef} className="chat-field" value={text} onChange={(e) => setText(e.target.value)}
