// hud.jsx — weightless floating game HUD. Widgets toggle from the Toolbox;
...
function DraggableFloat({ widgetKey, pos, custom, dragMode, onDrag, onCommit, onGuides, anim, children }) {
  const ref = useRef(null);
  const st = useRef(null);
  const raf = useRef(0);
  const style = custom
    ? { left: custom.x, top: custom.y, right: "auto", transform: "none" }
    : pos;
  const begin = (e) => {
    if (!dragMode) return;
    ...
    document.querySelectorAll(".hud .float").forEach((o) => {
      if (o === el) return; const b = o.getBoundingClientRect();
      xs.push(b.left, b.left + b.width / 2, b.right);
      ys.push(b.top, b.top + b.height / 2, b.bottom);
    });
    const vw = window.innerWidth, vh = window.innerHeight;
    xs.push(vw / 2, 20, vw - 20); ys.push(vh / 2, 20, vh - 20);
  ...
  const move = (e) => {
    ...
    s.last = { x: nl, y: nt }; s.moved = true; s.guides = guides;
    if (!raf.current) raf.current = requestAnimationFrame(() => { raf.current = 0; onDrag(widgetKey, s.last); onGuides && onGuides(s.guides); });
  };
  const end = (e) => {
    if (!st.current) return;
    const el = ref.current; el.classList.remove("dragging");
    try { el.releasePointerCapture(e.pointerId); } catch (_) {}
    if (st.current.moved) onCommit(widgetKey, st.current.last);
    st.current = null; onGuides && onGuides([]);
  };
  return (
    <div ref={ref} className={`float ${dragMode ? "draggable" : ""}`} style={style}
      onPointerDown={begin} onPointerMove={move} onPointerUp={end} onPointerCancel={end}>
      <div className="floatI" style={{ animation: dragMode ? "none" : anim }}>{children}</div>
    </div>
  );
}

function DaysCard({ density, onClick }) {
  return (
    <div className="card glass tappable" onClick={onClick}>
      <div className="lbl">在一起</div>
      <div className="days-row">
        <span className="num days-n">365</span><span className="days-u">天</span>
        <span className="days-heart"><IHeart size={17} fill="currentColor" sw={0} /></span>
      </div>
      {density === "rich" && <div className="sub" style={{ marginTop: 6 }}>从 2025.6.4 起</div>}
    </div>
  );
}
function MemoryCard({ density, onClick }) {
  return (
    <div className="card glass tappable" onClick={onClick}>
      <div className="mem">
...
function AnniversaryChip({ onClick }) {
  return (
    <div className="chip glass tappable" onClick={onClick}>
      <span className="ic"><ISparkle size={16} /></span>
      <span>距纪念日还有 <b className="num">18</b> 天</span>
    </div>
  );
}
function AmbientPill({ density, weather, nowTs, onClick }) {
  ...
  return (
    <div className="ambient glass tappable" onClick={onClick}>
      <span className="t num">{hh}:{mm}</span><span className="div" />
      <span className="w"><span className="ic"><WIcon size={16} /></span>{weather.temp}°{density === "rich" ? ` · ${weather.label}` : ""}</span>
    </div>
  );
}
function Minimap({ label, onClick }) {
  return (
    <div className="minimap glass tappable" onClick={onClick} title="切换空间">
      <div className="mini-room"><RoomArt shadow={false} /></div>
      <div className="ringdot" />
      <div className="lbl-m">{label || "我们的房间"}</div>
    </div>
  );
}
function LightingToggle({ mood, onChange }) {
  const opts = [
    ...
    <div className="lighting glass">
      {opts.map(({ k, label, Icon }) => (
        <button key={k} className={mood === k ? "on" : ""} onClick={() => onChange(k)}>
          <span className="ic"><Icon size={15} /></span>{label}
        </button>
      ))}
    </div>
  );
...
const WIDGET_SOON = [
  { key: "custom", label: "自创组件", Icon: IWand, c: "linear-gradient(135deg,#BFE6FA,#9FD6F4)" },
];

function Toolbox({ open, setOpen, widgets, setWidget, dragMode, setDragMode, resetPos }) {
  return (
    <div className="tb-wrap">
      <div className={`tb-panel glass ${open ? "show" : ""}`}>
        <div className="tb-hd">
          <h4><span className="tb-wand"><IWand size={18} /></span>自定义界面</h4>
          <p>开关组件 · 解锁编辑后可拖动摆放</p>
        </div>
        <div className="tb-list">
          <div className="tb-row edit" onClick={() => setDragMode((d) => !d)}>
            <span className="tb-ic" style={{ background: dragMode ? "linear-gradient(135deg,#9FD6F4,#5FB0E2)" : "var(--glass-bg-2)", color: dragMode ? "#fff" : "var(--accent-deep)" }}><IMove size={16} /></span>
            <span className="tb-nm">解锁编辑 · 拖动摆放</span>
            <span className={`sw ${dragMode ? "on" : ""}`}><i /></span>
          </div>
          {dragMode && <div className="tb-reset" onClick={resetPos}>恢复默认位置</div>}

          <div className="tb-sec">必备组件</div>
          {WIDGET_REQUIRED.map((w) => (
            <div key={w.key} className="tb-row req">
              <span className="tb-ic" style={{ background: w.c }}><w.Icon size={16} /></span>
              <span className="tb-nm">{w.label}</span>
              <span className="tb-lock"><ILock size={14} /></span>
            </div>
          ))}
          ...
          {WIDGET_ADDONS.map((w) => (
            <div key={w.key} className="tb-row live" onClick={() => setWidget(w.key, !widgets[w.key])}>
              <span className="tb-ic" style={{ background: w.c }}><w.Icon size={16} /></span>
              <span className="tb-nm">{w.label}</span>
              <span className={`sw ${widgets[w.key] ? "on" : ""}`}><i /></span>
            </div>
          ))}
      ...
      <button className={`fab ${open ? "open" : ""}`} onClick={() => setOpen((o) => { const n = !o; if (!n) setDragMode(false); return n; })} aria-label="自定义界面">
...
function hudPositions(layout) {
  const C = { left: "50%", transform: "translateX(-50%)" };
  const RAIL = { top: "50%", left: 14, transform: "translateY(-50%)" };
  if (layout === "cluster") {
    return { minimap: { top: 18, left: 18 }, presence: RAIL, ambient: { top: 22, right: 20 },
      memory: { top: 96, right: 20 }, days: { top: 204, right: 20 }, anniv: { top: 312, right: 20 }, music: { left: 18, bottom: 22 } };
  }
  if (layout === "topbar") {
    return { minimap: { top: 18, left: 18 }, ambient: { top: 22, right: 20 }, presence: RAIL,
      anniv: { top: 24, ...C }, days: { top: 64, ...C }, memory: { top: 108, right: 20 }, music: { left: 92, bottom: 22 } };
  }
  return { minimap: { top: 18, left: 18 }, ambient: { top: 22, right: 20 }, presence: RAIL,
    anniv: { top: 26, ...C }, days: { top: 200, right: 20 }, memory: { top: 96, right: 20 }, music: { left: 92, bottom: 22 } };
}

function HUD({ layout, mood, density, weather, nowTs, widgets, setWidget, tbOpen, setTbOpen, setMood, onNavigate, spaceName }) {
  const p = hudPositions(layout);
  const [dragMode, setDragMode] = useState(false);
  const [wpos, setWpos] = useState(() => { try { return JSON.parse(localStorage.getItem("ow-wpos-v1") || "{}"); } catch (_) { return {}; } });
  const [guides, setGuides] = useState([]);
  const onDrag = (k, pt) => setWpos((o) => ({ ...o, [k]: pt }));
  const onCommit = (k, pt) => setWpos((o) => { const n = { ...o, [k]: pt }; try { localStorage.setItem("ow-wpos-v1", JSON.stringify(n)); } catch (_) {} return n; });
  const resetPos = () => { setWpos({}); try { localStorage.removeItem("ow-wpos-v1"); } catch (_) {} };
  const nav = (k) => (dragMode ? undefined : () => onNavigate(k));
  const df = (key, anim) => ({ widgetKey: key, pos: p[key], custom: wpos[key], dragMode, onDrag, onCommit, onGuides: setGuides, anim });
  return (
    <div className="hud">
      <HudStyles />
      {widgets.minimap && <DraggableFloat {...df("minimap", "floatC 7s ease-in-out infinite")}><Minimap label={spaceName} onClick={nav("space")} /></DraggableFloat>}
      {widgets.ambient && <DraggableFloat {...df("ambient", "floatA 6.5s ease-in-out infinite .4s")}><AmbientPill density={density} weather={weather} nowTs={nowTs} onClick={nav("clock")} /></DraggableFloat>}
      {widgets.anniv && <DraggableFloat {...df("anniv", "floatB 6s ease-in-out infinite .2s")}><AnniversaryChip onClick={nav("calendar")} /></DraggableFloat>}
      {widgets.days && <DraggableFloat {...df("days", "floatA 7.5s ease-in-out infinite")}><DaysCard density={density} onClick={nav("timeline")} /></DraggableFloat>}
      {widgets.memory && <DraggableFloat {...df("memory", "floatB 6.8s ease-in-out infinite .6s")}><MemoryCard density={density} onClick={nav("photos")} /></DraggableFloat>}
      {widgets.music && <DraggableFloat {...df("music", "floatA 7.2s ease-in-out infinite .3s")}><MusicPlayer spaceName={spaceName} /></DraggableFloat>}
      {widgets.lighting && <LightingToggle mood={mood} onChange={setMood} />}
      {dragMode && <div className="edit-banner glass" onClick={() => setDragMode(false)} title="完成编辑"><span className="ic"><IMove size={15} /></span>拖动摆放组件 · 点此完成<span className="eb-done"><ICheck size={14} /></span></div>}
      ...
      <Toolbox open={tbOpen} setOpen={setTbOpen} widgets={widgets} setWidget={setWidget}
        dragMode={dragMode} setDragMode={setDragMode} resetPos={resetPos} />
    </div>
  );
}
