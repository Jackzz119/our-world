// settings.jsx — 设置 modal. Reuses the .modal.mini shell + .glass + .sw switch.
...
const gload = (k, fb) => { try { const v = localStorage.getItem(k); return v ? { ...fb, ...JSON.parse(v) } : fb; } catch { return fb; } };
...
const SettingsStyles = () => (
  ...
  /* expandable password block */
  .set-expand{overflow:hidden;transition:height .3s cubic-bezier(.3,.8,.35,1);}
  .set-pw{padding:4px 15px 15px;display:flex;flex-direction:column;gap:10px;}
...
function Segmented({ opts, value, onChange, withIcon }) {
  return (
    <div className="seg">
      {opts.map((o) => (
        <button key={o.k} className={value === o.k ? "on" : ""} onClick={() => onChange(o.k)}>
          {withIcon && o.Icon && <span className="ic"><o.Icon size={14} /></span>}
          {o.label}
        </button>
      ))}
    </div>
...
function PersonRow({ slotId, color, initial, role, name, onName }) {
  return (
    <div className="set-row">
      ...
      <div className="set-body">
        <input className="set-edit" value={name} onChange={(e) => onName(e.target.value)}
          spellCheck={false} maxLength={12} aria-label="昵称" />
      </div>
...
function SettingsScreen({ open, onClose, t, setTweak, profile, setP }) {
  const p = profile;
  const set = (k, v) => setP((o) => ({ ...o, [k]: v }));
  const [pwOpen, setPwOpen] = useGS(false);
  const [pw, setPw] = useGS({ cur: "", a: "", b: "" });
  const [saved, setSaved] = useGS(false);
  const pwRef = useGR(null);

  const pwValid = pw.cur && pw.a.length >= 4 && pw.a === pw.b;
  const savePw = () => {
    if (!pwValid) return;
    setSaved(true); setPw({ cur: "", a: "", b: "" });
    setTimeout(() => { setSaved(false); setPwOpen(false); }, 1400);
  };
  const pwH = pwRef.current ? pwRef.current.scrollHeight : 0;
    ...
    <>
      ...
      <div className={`modal mini glass ${open ? "show" : ""}`} aria-hidden={!open}>
        <div className="modal-hd">
          <span className="si" style={{ background: "linear-gradient(135deg,#C9D6F0,#9AA8D6)" }}><ICog size={19} /></span>
          <h2>设置</h2>
        ...
        <div className="modal-body">
          ...
          {/* ── 个人资料 ── */}
          <div className="set-label"><span className="ic"><IUser size={14} /></span>个人资料</div>
          <div className="set-group glass">
            <div className="set-row">
              <span className="set-ico" style={{ background: "linear-gradient(135deg,#BFE6FA,#6FBCE8)" }}><IHeart size={16} fill="#fff" sw={0} /></span>
              <div className="set-body">
                <input className="set-edit world" value={p.world} onChange={(e) => set("world", e.target.value)}
                  spellCheck={false} maxLength={16} aria-label="世界名称" />
          ...
          <div className="set-group glass" style={{ marginTop: 10 }}>
            <PersonRow slotId="set-ava-her" color="linear-gradient(135deg,#F8C8D6,#EF9DB4)"
              initial={p.her.slice(0, 1)} role="她" name={p.her} onName={(v) => set("her", v)} />
            <PersonRow slotId="set-ava-me" color="linear-gradient(135deg,#FCD9A0,#F1B45A)"
              initial={p.me.slice(0, 1)} role="他" name={p.me} onName={(v) => set("me", v)} />
            <div className="set-row">
              <span className="set-ico" style={{ background: "linear-gradient(135deg,#D8C2F0,#A98FD6)" }}><ISparkle size={15} /></span>
              <div className="set-body"><div className="set-t">在一起的那天</div><div className="set-s">从这天开始数我们的日子</div></div>
              <input className="set-date" type="date" value={p.anniv} onChange={(e) => set("anniv", e.target.value)} aria-label="纪念日" />
            </div>
          </div>

          {/* ── 账号与密码 ── */}
          <div className="set-label"><span className="ic"><IShield size={14} /></span>账号与密码</div>
          <div className="set-group glass">
            <div className="set-row">
              <span className="set-ico" style={{ background: "linear-gradient(135deg,#AEDFF2,#7CC6EC)" }}><IMail size={16} /></span>
              <div className="set-body">
                <input className="set-edit" value={p.email} onChange={(e) => set("email", e.target.value)}
                  spellCheck={false} aria-label="绑定邮箱" />
                <div className="set-s" style={{ paddingLeft: 8 }}>绑定的邮箱</div>
              </div>
            </div>
            <div className="set-row tap" onClick={() => setPwOpen((o) => !o)}>
              <span className="set-ico" style={{ background: "linear-gradient(135deg,#FBE6A8,#F1C75A)" }}><IKey size={16} /></span>
              <div className="set-body"><div className="set-t">修改密码</div><div className="set-s">{pwOpen ? "输入旧密码与新密码" : "上次更新于 3 个月前"}</div></div>
              <span className="set-chev" style={{ transform: pwOpen ? "rotate(90deg)" : "none", transition: "transform .25s" }}><IChevron size={17} /></span>
            </div>
            <div className="set-expand" style={{ height: pwOpen ? pwH : 0 }}>
              <div className="set-pw" ref={pwRef}>
                <input type="password" placeholder="当前密码" value={pw.cur} onChange={(e) => setPw((s) => ({ ...s, cur: e.target.value }))} />
                <input type="password" placeholder="新密码（至少 4 位）" value={pw.a} onChange={(e) => setPw((s) => ({ ...s, a: e.target.value }))} />
                <input type="password" placeholder="再次输入新密码" value={pw.b} onChange={(e) => setPw((s) => ({ ...s, b: e.target.value }))} />
                <div className="row">
                  <span className={`ok ${saved ? "show" : ""}`}><ICheck size={14} />已更新</span>
                  <button className="btn-save" onClick={savePw} disabled={!pwValid}>保存</button>
                </div>
              </div>
            </div>
            <div className="set-row">
              <span className="set-ico" style={{ background: "linear-gradient(135deg,#C9E8C2,#86C99A)" }}><ILock size={16} /></span>
              <div className="set-body"><div className="set-t">应用锁</div><div className="set-s">{p.lock ? "进入小世界需要密码" : "关闭后无需验证即可进入"}</div></div>
              <span className={`sw ${p.lock ? "on" : ""}`} onClick={() => set("lock", !p.lock)}><i /></span>
            </div>
          </div>

          {/* ── 主题外观 ── */}
          <div className="set-label"><span className="ic"><IPaint size={14} /></span>主题外观</div>
          <div className="set-group glass">
            <div className="set-row">
              <div className="set-body"><div className="set-t">玻璃质感</div><div className="set-s">界面卡片的材质</div></div>
              <Segmented opts={GLASS_OPTS} value={t.glassStyle} onChange={(k) => setTweak("glassStyle", k)} />
            </div>
            <div className="set-row" style={{ flexWrap: "wrap" }}>
              <div className="set-body"><div className="set-t">光线时段</div><div className="set-s">一天里的光与氛围</div></div>
              <Segmented opts={MOOD_OPTS} value={t.mood} onChange={(k) => setTweak("mood", k)} withIcon />
            </div>
          </div>

          <div className="set-foot">{p.world} · v1.0 · 只属于你们<span className="hh"><IHeart size={11} fill="currentColor" sw={0} /></span></div>
        </div>
      </div>
    </>
  );
...
Object.assign(window, { SettingsScreen, PROFILE_DEFAULT, gload });
