// music.jsx — 一起听歌: floating music widget (addon). One taps play, everyone in
...
const { useState: useMu, useEffect: useMuE, useRef: useMuR } = React;

const TRACKS = [
  { title: "云朵上的下午", artist: "小满 & 知夏", root: 261.63, chord: [0, 4, 7, 11], dur: 214, cover: "linear-gradient(145deg,#FCE3B0,#F5B774)" },
  { title: "雨天的窗边", artist: "Lo-fi 时光", root: 220.00, chord: [0, 3, 7, 10], dur: 198, cover: "linear-gradient(145deg,#BFD0F2,#8C9DDB)" },
  { title: "暖灯电台", artist: "夜晚频率", root: 196.00, chord: [0, 4, 7, 11], dur: 236, cover: "linear-gradient(145deg,#E7C4F0,#B68FD9)" },
  { title: "一起散步", artist: "周末", root: 293.66, chord: [0, 5, 7, 12], dur: 188, cover: "linear-gradient(145deg,#BFE8D2,#86C9A6)" },
];
const muLoad = () => { try { return JSON.parse(localStorage.getItem("ow-music-v1") || "null") || {}; } catch { return {}; } };
...
function MusicPlayer({ spaceName, partner, onClick }) {
  const init = muLoad();
  const [i, setI] = useMu(typeof init.i === "number" ? init.i % TRACKS.length : 0);
  const [playing, setPlaying] = useMu(false);
  const [pos, setPos] = useMu(typeof init.pos === "number" ? init.pos : 0);
  const [muted, setMuted] = useMu(!!init.muted);
  const audio = useMuR(null);
  const t = TRACKS[i];

  // persist (not playing — never autoplay sound without a gesture)
  useMuE(() => { try { localStorage.setItem("ow-music-v1", JSON.stringify({ i, pos, muted })); } catch {} }, [i, pos, muted]);
  ...
  useMuE(() => { if (playing && pos >= t.dur) next(); }, [pos, playing]);
  ...
  const ensure = () => {
    ...
    lfo.frequency.value = 0.18; lfoGain.gain.value = 0.18; lfo.connect(lfoGain); lfoGain.connect(master.gain); lfo.start();
    const voices = [0, 1, 2, 3].map(() => { const o = ctx.createOscillator(); o.type = "triangle"; const g = ctx.createGain(); g.gain.value = 0.25; o.connect(g); g.connect(master); o.start(); return { o, g }; });
    audio.current = { ctx, master, lp, voices, started: true };
    return audio.current;
  };
  const applyChord = (a) => {
    if (!a) return;
    t.chord.forEach((semi, k) => { const v = a.voices[k]; if (v) v.o.frequency.setValueAtTime(t.root * Math.pow(2, semi / 12) / (k === 0 ? 2 : 1), a.ctx.currentTime); });
  };
  const ramp = (a, on) => { if (!a) return; const target = on && !muted ? 0.14 : 0; a.master.gain.cancelScheduledValues(a.ctx.currentTime); a.master.gain.setTargetAtTime(target, a.ctx.currentTime, on ? 0.5 : 0.25); };

  useMuE(() => { const a = audio.current; if (a) applyChord(a); }, [i]);
  useMuE(() => { const a = audio.current; if (a) ramp(a, playing); }, [muted]);
  useMuE(() => () => { try { audio.current && audio.current.ctx.close(); } catch {} }, []);

  const toggle = (e) => {
    e && e.stopPropagation();
    const a = ensure();
    if (!playing) { if (a) { a.ctx.resume(); applyChord(a); ramp(a, true); } setPlaying(true); }
    else { ramp(a, false); setPlaying(false); }
  };
  const next = () => { setI((x) => (x + 1) % TRACKS.length); setPos(0); };
  const prev = () => { if (pos > 3) { setPos(0); } else { setI((x) => (x - 1 + TRACKS.length) % TRACKS.length); setPos(0); } };
  const seek = (e) => { const r = e.currentTarget.getBoundingClientRect(); const f = Math.min(1, Math.max(0, (e.clientX - r.left) / r.width)); setPos(Math.round(f * t.dur)); };
    ...
    <div className="card mp glass">
      ...
      <div className="mp-top">
        <div className={`mp-cover ${playing ? "spin" : ""}`} style={{ background: t.cover }}>
          <IMusic size={20} /><span className="hole" />
        </div>
        <div className="mp-info">
          <div className="mp-title">{t.title}</div>
          <div className="mp-artist">{t.artist}</div>
          <div className="mp-share">
            <span className="ic"><IHeadset size={13} /></span>
            {spaceName || "我们的房间"} · 一起听
            <span className={`mp-eq ${playing && !muted ? "on" : ""}`}><i /><i /><i /><i /></span>
          </div>
        </div>
      </div>

      <div className="mp-prog" onClick={stop(() => {})}>
        <div className="mp-bar" onClick={seek}><div className="mp-fill" style={{ width: pct + "%" }} /></div>
        <div className="mp-times"><span>{fmt(pos)}</span><span>{fmt(t.dur)}</span></div>
      </div>

      <div className="mp-ctrl-wrap">
        <div className="mp-ctrl">
          <button className="mp-btn" onClick={stop(prev)} aria-label="上一首"><ISkipB size={22} /></button>
          <button className="mp-btn mp-play" onClick={toggle} aria-label={playing ? "暂停" : "播放"}>{playing ? <IPause size={18} /> : <IPlay size={18} />}</button>
          <button className="mp-btn" onClick={stop(next)} aria-label="下一首"><ISkipF size={22} /></button>
        </div>
        <button className="mp-btn mp-vol" onClick={stop(() => setMuted((m) => !m))} aria-label={muted ? "取消静音" : "静音"} title={muted ? "取消静音" : "静音"}>
