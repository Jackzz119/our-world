// calendar.jsx — real Calendar (约会 / 纪念日) + Clock·Alarm screens.
// Reuse the .modal shell from screens.jsx (ScreenStyles is always mounted).
const { useState: useKS, useEffect: useKE, useRef: useKR } = React;
...
const ksave = (k, v) => { try { localStorage.setItem(k, JSON.stringify(v)); } catch {} };
...
/* ════════ CALENDAR ════════ */
function CalendarScreen({ open, onClose, events, setEvents }) {
  const today = new Date();
  const tY = today.getFullYear(), tM = today.getMonth(), tD = today.getDate();
  const [cur, setCur] = useKS({ y: tY, m: tM });
  const [sel, setSel] = useKS(null);
  const [title, setTitle] = useKS("");
  ...
  const nextAnniv = () => {
    let y = tY; let dn = new Date(y, ANNIV.m, ANNIV.d);
    if (dn < new Date(tY, tM, tD)) dn = new Date(++y, ANNIV.m, ANNIV.d);
    const diff = Math.round((dn - new Date(tY, tM, tD)) / 864e5);
    return { diff, y };
  };
  ...
  const cntdown = (dstr) => {
    ...
    const diff = Math.round((new Date(y, m - 1, d) - new Date(tY, tM, tD)) / 864e5);
  ...
  for (let d = 1; d <= days; d++) {
    ...
    cells.push(
      <div key={ds} className={`cal-cell ${isToday ? "today" : ""} ${isAnniv ? "anniv" : ""} ${sel === ds ? "sel" : ""}`}
        onClick={() => setSel(ds)}>
        {d}
        <div className="dots">{isAnniv && <i className="an" />}{evs && <i className="ev" />}</div>
      </div>
    );
  }
    ...
    <>
      ...
      <div className={`modal mini glass ${open ? "show" : ""}`} aria-hidden={!open}>
        <div className="modal-hd">
          <span className="si" style={{ background: "linear-gradient(135deg,#D8C2F0,#A98FD6)" }}><ICalendar size={19} /></span>
          <h2>日历 · 约会</h2>
          <button className="modal-x" onClick={onClose} aria-label="关闭"><IClose size={17} /></button>
        </div>
        <div className="modal-body">
          <div className="cal-anniv glass">
            <span className="ring"><IHeart size={24} fill="#fff" sw={0} /></span>
            <div className="ct">
              <div className="l">距下一个纪念日</div>
              <div className="n">在一起满 {yearsTogether} 周年 · {annivYear}.6.4</div>
            </div>
            <div className="big">{annivDiff}<small>天</small></div>
          </div>

          <div className="cal-nav">
            <button onClick={() => move(-1)} aria-label="上个月"><IChevron size={18} style={{ transform: "rotate(180deg)" }} /></button>
            <span className="mlabel">{monthLabel}</span>
            <button onClick={() => move(1)} aria-label="下个月"><IChevron size={18} /></button>
          </div>
          <div className="cal-grid">
            {WK.map((w) => <div key={w} className="cal-wk">{w}</div>)}
            {cells}
          </div>
          ...
          <div className="cal-add">
            ...
            <input value={title} onChange={(e) => setTitle(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && addEvent()} placeholder="添加一个约会…" />
            <button onClick={addEvent} disabled={!sel || !title.trim()} aria-label="添加"><IPlus size={20} /></button>
          </div>

          <div className="cal-sec">即将到来的约会</div>
          {upcoming.length === 0 && <div style={{ color: "var(--glass-sub)", fontSize: 12.5, textAlign: "center", padding: "10px 0" }}>还没有计划，点日期添加一个吧 ·</div>}
          {upcoming.map((e) => {
            const [, m, d] = e.date.split("-");
            return (
              <div className="ev-row glass" key={e.id}>
                <div className="dt"><div className="d">{Number(d)}</div><div className="mo">{Number(m)} 月</div></div>
                <div className="et">{e.title}</div>
                <div className="cd">{cntdown(e.date)}</div>
                <button className="del" onClick={() => delEvent(e.id)} aria-label="删除"><IClose size={15} /></button>
              </div>
            );
          })}
        </div>
...
/* ════════ CLOCK · ALARM ════════ */
function ClockScreen({ open, onClose, nowTs, weather, alarms, setAlarms }) {
  const d = new Date(nowTs);
  const hh = pad(d.getHours()), mm = pad(d.getMinutes()), ss = pad(d.getSeconds());
  const wd = ["周日", "周一", "周二", "周三", "周四", "周五", "周六"][d.getDay()];
  const dateStr = `${d.getMonth() + 1} 月 ${d.getDate()} 日 · ${wd}`;
  const [atime, setAtime] = useKS("07:30");
  const [alabel, setAlabel] = useKS("");
  ...
  const del = (id) => setAlarms(alarms.filter((a) => a.id !== id));
  const add = () => {
    if (!atime) return;
    setAlarms([...alarms, { id: "a" + Date.now(), time: atime, label: alabel.trim() || "提醒", on: true }]
  ...
  return (
    <>
      ...
      <div className={`modal mini glass ${open ? "show" : ""}`} aria-hidden={!open}>
        <div className="modal-hd">
          <span className="si" style={{ background: "linear-gradient(135deg,#BFE6FA,#6FBCE8)" }}><IClock size={19} /></span>
          <h2>时间 · 闹钟</h2>
          <button className="modal-x" onClick={onClose} aria-label="关闭"><IClose size={17} /></button>
        </div>
        <div className="modal-body">
          <div className="clock-face">
            <div className="big num">{hh}:{mm}<span className="s">{ss}</span></div>
            <div className="sub">
              <span>{dateStr}</span>
              <span className="wx"><WIcon size={16} />{weather.label} {weather.temp}°{weather.place ? ` · ${weather.place}` : ""}</span>
            </div>
          </div>
          ...
          {alarms.map((a) => (
            <div className={`alarm glass ${a.on ? "" : "off"}`} key={a.id}>
              ...
              <div className="body"><div className="lab">{a.label}</div></div>
              <span className={`sw ${a.on ? "on" : ""}`} onClick={() => toggle(a.id)}><i /></span>
              <button className="del" onClick={() => del(a.id)} aria-label="删除"><IClose size={16} /></button>
            </div>
          ))}
          <div className="alarm-add">
            <input type="time" value={atime} onChange={(e) => setAtime(e.target.value)} />
            <input type="text" value={alabel} onChange={(e) => setAlabel(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && add()} placeholder="提醒内容…" />
            <button onClick={add} aria-label="添加闹钟"><IPlus size={20} /></button>
          </div>
        </div>
      </div>
    </>
...
Object.assign(window, { CalendarScreen, ClockScreen });
