// scene.jsx — generated isometric room "diorama" (replaces the upload slot).
// A small iso-projection helper builds a cozy bedroom-study. The mood overlays
// in the HTML still tint this for twilight / golden / night lighting.
// RoomArt is exported so the minimap can reuse it.

const ROOM = {
  floorTop: "#ECDCC4", floorBase: "#C9B193",
  wallA: "#F3ECE0", wallB: "#E4D9C7",          // lit / shaded wall
  woodT: "#CBA47B", woodR: "#B68F66", woodL: "#A07B55",
  creamT: "#FBFCFE", creamR: "#E7EFF8", creamL: "#D6E2F1",
  blushT: "#F8D7DF", blushR: "#EFC2CE", blushL: "#E3AEBD",
  skyT: "#BFE3F5", skyR: "#9FCDEA", skyL: "#83B7DD",
  rug: "#AEDFF2",
};

function RoomArt({ shadow = true }) {
  const ux = 78, uy = 43, vx = -78, vy = 43, hz = 47, OX = 500, OY = 156;
  const X = (u, v) => OX + u * ux + v * vx;
  const Y = (u, v, w = 0) => OY + u * uy + v * vy - w * hz;
  const pt = (u, v, w = 0) => `${X(u, v)},${Y(u, v, w)}`;
  const f = (pts) => pts.map((p) => pt(p[0], p[1], p[2] || 0)).join(" ");

  // isometric box → 3 faces (left=v1, right=u1, top=w1)
  const Box = ({ u0, u1, v0, v1, w0, w1, t, r, l }) => (
    <g>
  ...
  const Shadow = ({ u, v, rx = 34, ry = 17, o = 0.16 }) => (
    <ellipse cx={X(u, v)} cy={Y(u, v)} rx={rx} ry={ry} fill={`rgba(40,42,60,${o})`} />
  );

  const H = 2.5; // wall height
  return (
    <svg viewBox="0 0 1000 720" width="100%" height="100%" preserveAspectRatio="xMidYMid meet"
      style={{ display: "block", overflow: "visible" }}>
      <defs>
        <linearGradient id="winGlass" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0" stopColor="#FCEFC9" /><stop offset=".5" stopColor="#CFE6F6" /><stop offset="1" stopColor="#A7C6E6" />
        </linearGradient>
        <radialGradient id="lampGlow" cx=".5" cy=".5" r=".5">
          <stop offset="0" stopColor="#FFF0BE" stopOpacity=".95" /><stop offset="1" stopColor="#FFF0BE" stopOpacity="0" />
        </radialGradient>
        <linearGradient id="screen" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stopColor="#8FD0F0" /><stop offset="1" stopColor="#5E97C8" />
        </linearGradient>
      ...
      <g filter={shadow ? "url(#roomShadow)" : undefined}>
        ...
        {/* ── rug ── */}
        <polygon points={f([[0.7, 1.0, 0.012], [2.9, 1.0, 0.012], [2.9, 3.1, 0.012], [0.7, 3.1, 0.012]])}
          fill={ROOM.rug} opacity="0.82" />
        <polygon points={f([[1.0, 1.3, 0.014], [2.6, 1.3, 0.014], [2.6, 2.8, 0.014], [1.0, 2.8, 0.014]])}
...
function RoomScene({ weather = "cloud" }) {
  return (
    <div className="scene-base" data-wx={weather}>
      <div className="scene-ambient" />
      <div className="room-stage"><RoomArt /></div>
...
Object.assign(window, { RoomScene, RoomArt });
