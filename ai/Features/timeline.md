...
## 七、v2 重做：Codex 策划 + 三方向比稿（2026-09-05，待用户拍板）
...
2. 是否改名「回忆日记」
3. 三 tab 拆分为三个物件各自 surface（连带照片墙、心愿单）
4. 卡片底色：白卡 + 作者色边线（Claude 建议）vs 淡蓝/淡粉底（Codex 图）
5. 纸页宽度：480–520px（Claude）vs 560–590px（Codex 规格）
6. `--accent-deep` 统一为 `#2F9AD3`（D-9）还是保留 `#268FBE`
