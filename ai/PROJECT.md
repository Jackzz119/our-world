...
> 最后更新：2026-09-07（随光磨砂导航已获用户认可；棕皮旧纸书本已接入竖直翻页、连续翻阅与阅读时雨不停播；拿起/收回与羽毛笔交互仍待办，尚未部署）
...
- **timeline / 我们的日记**（2026-09-07）：棕皮旧纸居中双页接入真实图文随纸竖直翻动、连续翻阅与阅读时雨不停播；羽毛笔仍为静态图片。验收、加载边界与数据约束见 `ai/Features/timeline.md`，设计状态见 `ai/design_system/cinnaglass/ui-system.html#journal-turn`。
- **随光磨砂导航**（2026-09-07）：五入口、三时辰中性透明细纹、悬停/按压暖光与导航小菜单已接入；其他功能弹窗未迁移，详见 `ai/Features/navigation-glass.md`。
