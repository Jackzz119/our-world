...
// Ported from the design prototype. Persistence swapped from the omelette
// sidecar to localStorage so dropped photos survive reloads in production.
//
// Attributes: id (persistence key) · shape (rect|rounded|circle|pill) ·
// radius · mask · fit (cover|contain|fill) · position · placeholder · src.

(() => {
    const STORE_KEY = 'ow-image-slots-v1';
    const MAX_DIM = 1200;
    ...
    let slots = {};
    try {
        slots = JSON.parse(localStorage.getItem(STORE_KEY) || '{}') || {};
    } catch {
        slots = {};
    }
    ...
    class ImageSlot extends HTMLElement {
        ...
        constructor() {
            ...
            root.innerHTML =
                ...
                icon +
                '    <div class="cap"></div>' +
                '    <div class="sub">拖入图片 · 或 <u>选择文件</u></div></div>' +
        ...
        async _ingest(file) {
            ...
            if (!file || ACCEPT.indexOf(file.type) < 0) {
                this._setError('请拖入 PNG / JPEG / WebP / AVIF 图片。');
                return;
            }
            const gen = ++this._gen;
            try {
        ...
        _render() {
            ...
            // localStorage is always writable → slots are editable in production.
            const editable = true;
            ...
            this._cap.textContent = this.getAttribute('placeholder') || '拖入图片';
            if (url) {
