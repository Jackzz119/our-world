// sidebar.tsx — Discord-style left pop-out: world identity · room channels ·
...
import { IChevron, ICog, IDusk, IHash, IMic, IMicOff, IMoon, ISun, IVolume } from './icons';
import { ROOM_ICONS, VOICE_DEFAULT } from './rooms';
import type { Mood, SetTweak } from './tweaks';
import type { Profile, Room } from './model';

const MOODS_SB: { k: Mood; label: string; Icon: (p: IcoProps) => ReactNode }[] = [
...
const SidebarStyles = () => (
  ...
  /* room config inline (grid-rows trick — animates open/close without JS height measurement) */
  .sb-cfg{display:grid;grid-template-rows:0fr;transition:grid-template-rows .3s cubic-bezier(.3,.8,.35,1);}
  .sb-cfg.open{grid-template-rows:1fr;}
  .sb-cfg-in{overflow:hidden;min-height:0;padding:10px 10px 6px;display:flex;flex-direction:column;gap:11px;}
...
export function Sidebar({
    ...
    const peopleIn = (rid: string) => [herRoom === rid ? her : null, meRoom === rid ? me : null].filter(Boolean) as Person[];
    const here = peopleIn(cur.id);
    const RICur = ROOM_ICONS[cur.icon] || IHash;

    const enterRoom = (r: Room) => enterSpace(r);
    const setCur = (patch: Partial<Room>) => setRooms((rs) => rs.map((r) => (r.id === cur.id ? { ...r, ...patch } : r)));
    ...
    return (
        <>
            ...
            <div className={`sb-dock ${open ? 'hide' : ''}`}>
                ...
                <div className="sb-pull" onClick={() => setOpen(true)} title="打开侧边栏">
                    <IChevron size={16} />
                </div>
            </div>
                ...
                <div className="owsb-card glass">
                    <div className="sb-hd">
                        <div className="sb-hd-top">
                            ...
                            <div className="sb-id">
                                ...
                                <div className="meta">
                                    <span className="dot" />在一起 <span className="num">{days}</span> 天 · 都在线
                                </div>
                            </div>
                            <div className="sb-close" onClick={() => setOpen(false)} title="收起">
                                <IChevron size={16} style={{ transform: 'rotate(180deg)' }} />
                            </div>
                        </div>
                        <div className="sb-here">
                            <span className="ic">
                                <RICur size={14} />
                            </span>
                            你在 · {cur.name}
                    ...
                    <div className="sb-scroll">
                        ...
                        <div className="sb-cat">
                            房间
                            <span className={`cog ${cfgOpen ? 'on' : ''}`} onClick={() => setCfgOpen((o) => !o)} title="房间设置">
                                <ICog size={14} />
                            </span>
                        </div>
                        {rooms.map((r) => {
                            ...
                            return (
                                <div key={r.id} className={`sb-room ${r.id === meRoom ? 'on' : ''}`} onClick={() => enterRoom(r)}>
                                    ...
                                    <span className="who">
                                        {ppl.map((pp) => (
                                            <span key={pp.id} className="mini" style={{ background: pp.color }}>
                                                {pp.ini}
                                            </span>
                                        ))}
                                    </span>
                        ...
                        <div className={`sb-cfg ${cfgOpen ? 'open' : ''}`}>
                            <div className="sb-cfg-in">
